...
def paths_to_dfs(paths: List[List[str]], my_country:str):
    ...
    for df in dfs:
        ...
        df_filtred['education'] = df['education']
        df_filtred['gender'] = df['gender']
...
def bar_chart(dotazniky: List[Tuple[pd.DataFrame, str]], choice: str, my_country:str, subtopic: bool, col=None, col_2=None):
    for operace in choice:
        ...
        if operace == 'mean':
            popisek = "Průměr"
            df['mean'] = df.groupby(['education', 'gender'])[work_with].transform('mean')
                
        elif operace == 'modus':
            popisek = "Modus"
            df['modus'] = df.groupby(['education', 'gender'])[work_with].transform(lambda x: pd.Series.mode(x)[0] if not pd.Series.mode(x).empty else None)
                
        elif operace == 'median':
            popisek = "Medián"
            df['median'] = df.groupby(['education', 'gender'])[work_with].transform('median')


        df_2 = df.drop_duplicates(subset=['education', operace], keep='first')
        df_2 = df_2.sort_values('education')
           
        if subtopic == False:
            title = f"{popisek} získaných bodů v dotazníku: {kompetence} \npro zemi: {country_code[my_country][0]} \nv období od {od} do {do}"
            filename = f"Barplot_G-EDU_{operace}_{kompetence}"

        else:
            title = f"{popisek} získaných bodů v dotazníku: {kompetence}, v podtématu: {col_to_name(col_2)} \npro zemi: {country_code[my_country][0]} \nv období od {od} do {do}"
            filename = f"Barplot_G-EDU_{operace}_{kompetence}_{col_to_name(col_2)}"
        ...
        plt.legend(fontsize='small', loc='lower center', ncol=3, bbox_to_anchor=(0.5, -0.25))
...
if make_for_subtopic == False:
    for idx, x in enumerate(my_dfs):
        x[0]['achieved'] = x[0].sum(axis=1, numeric_only=True)
        bar_chart(my_dfs[idx], choice, my_country, subtopic=False)
    ...
    for idx, x in enumerate(my_dfs):
        ...
        for col, col_2 in zip(points_cols, sub_topic_cols):
            bar_chart(my_dfs[idx], choice, my_country, subtopic=True, col=col, col_2=col_2)