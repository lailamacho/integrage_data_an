...
def hist(dotazniky: List[Tuple[pd.DataFrame, str]], choice: str, my_country:str, subtopic: bool, col=None, col_2=None):
    for operace in choice:
        ...
        if subtopic == False:
            ...
            filename = f"Hist_2D_{operace}_{kompetence}"

        else:
            title = f"{popisek} získaných bodů v závislosti na věku respondentů \nDotazník: {kompetence}, podtémma: {col_to_name(col_2)} \npro zemi: {country_code[my_country][0]} \nv období od {od} do {do}"
            filename = f"Hist_2D_{operace}_{kompetence}_{col_to_name(col_2)}"
        ...
        #        cbar=True,
        #        cbar_kws={'label': 'Počet respondentů'}
         #       )
        ...
        plt.colorbar(label='Počet respondentů')
