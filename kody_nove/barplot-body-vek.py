...
def bar_plot(dotazniky: List[Tuple[pd.DataFrame, str]], choice: str, my_country:str, subtopic: bool, col=None, col_2=None):
    for operace in choice:
        ...
        if subtopic == False:
            ...
            filename = f"Barplot_body-vek_{operace}_{kompetence}"

        else:
            title = f"{popisek} získaných bodů v závislosti na věku respondentů \nDotazník: {kompetence}, podtémma: {col_to_name(col_2)} \npro zemi: {country_code[my_country][0]} \nv období od {od} do {do}"
            filename = f"Barplot_body-vek_{operace}_{kompetence}_{col_to_name(col_2)}"
...
if make_for_subtopic == False:
    for idx, x in enumerate(my_dfs):
        ...
        bar_plot(my_dfs[idx], choice, my_country, subtopic=False)
    ...
    for idx, x in enumerate(my_dfs):
        ...
        for col, col_2 in zip(points_cols, sub_topic_cols):
            bar_plot(my_dfs[idx], choice, my_country, subtopic=True, col=col, col_2=col_2)